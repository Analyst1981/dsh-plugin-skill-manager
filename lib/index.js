...
function summarize(entry) {
	return {
		...
		...(entry.registered !== undefined ? { registered: entry.registered } : {}),
...
function renderEntryText(entry, previewChars) {
	...
	// The render path receives summarized entries (bodyPreview already cut);
	// the command path passes raw scan entries (full body). Accept either.
	const bodyText = entry.bodyPreview ?? previewBody(entry.body ?? "", previewChars);
	return [`## ${entry.name ?? basename(entry.path)} (${entry.format}, ${source})`, status, `Path: ${entry.path}`, "", bodyText].join("\n");
}
